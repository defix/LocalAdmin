...
If $sTxtPW = "" Or StringLen($sTxtPW) > 8 Or StringLen($sTxtPW) < 6 Or Not StringIsASCII($sTxtPW) Then
	...
	Exit
EndIf
$LA_pwdkey = StringEncrypt(1, $sTxtPW, $EncryptKey, $EncryptLevel, $EncryptFlag)
$sDEtest = StringEncrypt(0, $LA_pwdkey, $EncryptKey, $EncryptLevel, $EncryptFlag)
ClipPut($LA_pwdkey)
MsgBox(0, '密码：' & $sTxtPW, '加密字串（已复制到剪贴板）：' & @CRLF & $LA_pwdkey & @CRLF & @CRLF & '解密测试：' & $sDEtest)
